...
internal class CurveLineGraphDelegate(
    ...
    private fun PointF.toPixelPoint(interpolatedRange: FloatRange): PointF {
        val x = x.abscissaToPx(viewSize.width, interpolatedRange.start, interpolatedRange.endInclusive)
        val y = y.ordinateToPx(viewSize.height, currentMinY, currentMaxY)
        return PointF(x, y)
    }
